...
use crate::core::state::DockerContainer;
...
async fn poll_containers(docker: &Docker, store: &Store) -> Result<(), String> {
    ...
    for container in &container_list {
        ...
        tasks.push(async move {
            ...
            (name, image, status, state, stats)
        });
    }

    let joined_results = futures_util::future::join_all(tasks).await;

    for (name, image, status, state, (cpu_percent, mem_usage, mem_limit, net_rx, net_tx)) in joined_results {
